...
int totalrow = findTestData('Scenario 13/S13_TC040').getRowNumbers()

for (row = 1; row <= totalrow; row++) {
    String status_name = findTestData('Scenario 13/S13_TC040').getValue('status_name', row)

    String customer_code = findTestData('Scenario 13/S13_TC040').getValue('customer_code', row)

    String status_role = findTestData('Scenario 13/S13_TC040').getValue('status_role', row)

    String contract_id = findTestData('Scenario 13/S13_TC040').getValue('contract_id', row)
    ...
    WebUI.verifyElementNotPresent(findTestObject('Scenario 13/S13_TC040/input_Status Alert_add', [('alert_i') : i_alert]), 
        1)
