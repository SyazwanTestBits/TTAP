...
WebUI.verifyElementText(findTestObject('Scenario 10/S10_TC049/p_DT-CO Status'), 'Completed')
...
'NEED TO CHECK'
WebUI.verifyElementAttributeValue(findTestObject('Scenario 13/S13_TC049 TC054/input_status CO_verify'), 'value', findTestData(
        'Scenario 13/S13_TC079').getValue('CO status', 1), 0)
...
int numberrowtd = findTestData('Scenario 13/S13_TC079').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String colname : columnname) {
        String valuecol = findTestData('Scenario 13/S13_TC079').getValue(colname, rowl)
