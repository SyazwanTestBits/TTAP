...
for (def rowNum = 1; rowNum <= 4; rowNum++) {
    ...
    if (bookingNo || containerNo) {
		...
		cargoStatusExpected = testData.getValue('Cargo Status', rowNum)

        // Verify cargo status text 
        WebUI.verifyMatch(cargoStatusText, cargoStatusExpected, false, FailureHandling.STOP_ON_FAILURE)

        // If the verifyMatch fails, it will cause the script to fail
        KeywordUtil.logInfo("BookingNo: $bookingNo, ContainerNo: $containerNo, $cargoStatusExpected")
    }
}
