...
for (def rowNum2 = 1; rowNum2 <= 8; rowNum2++) {
    ...
    if (bookingNo || containerNo) {
        ...
        WebUI.verifyMatch(cargoStatusText, 'Vessel departure from T/S', false, FailureHandling.STOP_ON_FAILURE)

        KeywordUtil.logInfo("Row $rowNum2 - BookingNo: $bookingNo, ContainerNo: $containerNo, Vessel departure from T/S")
    }
}
