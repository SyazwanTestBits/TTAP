...
for (def rowNum2 = 1; rowNum2 <= 3; rowNum2++) {
    ...
    if (bookingNo || containerNo) {
        ...
        if ((rowNum2 == 1) || (rowNum2 == 2)) {
            WebUI.verifyMatch(cargoStatusText, 'Vessel departure from first POL', false, FailureHandling.STOP_ON_FAILURE)

            KeywordUtil.logInfo("Row $rowNum2 - BookingNo: $bookingNo, ContainerNo: $containerNo, Vessel departure from first POL")
        } else if (rowNum2 == 3) {
            WebUI.verifyMatch(cargoStatusText, 'Empty Container Release to Shipper', false, FailureHandling.STOP_ON_FAILURE)

            KeywordUtil.logInfo("Row $rowNum2 - BookingNo: $bookingNo, ContainerNo: $containerNo, Cargo status: Empty Container Release to Shipper")
        }
    }
}
