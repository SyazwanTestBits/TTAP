...
WebUI.callTestCase(findTestCase('0-Common/Login to Brivge'), [('url') : GlobalVariable.BRIVGE_URL, ('username') : GlobalVariable.BAF_USERNAME_LUQMAN
        , ('password') : GlobalVariable.BAF_PWD, ('verificationCode') : GlobalVariable.VERIFICATION_CODE, ('company') : GlobalVariable.COMPANY_SUPPLIER_1], 
...
CustomKeywords.'verifyExcelData.verifyDynamicSort'('C:\\Users\\luqma\\git\\tb-ttap-brivge-v2\\Excel Files\\Scenario 1\\Expected Data\\TC143\\Expected Sales Order.xlsx"', 
    downloadedFile, 2, [23, 24, 25], [1, 2, 15, 16, 21, 17])
...
int numberrowtd = findTestData('Scenario 1/S1_TC143-Parts Detail').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String colname : columnname) {
        String valuecol = findTestData('Scenario 1/S1_TC143-Parts Detail').getValue(colname, rowl)
