...
for (def rowNum = 1; rowNum <= 4; rowNum++) {
    ...
    if (bookingNo || containerNo) {
        ...
        if ((rowNum == 1) || (rowNum == 3)) {
            ...
            KeywordUtil.logInfo("Row $rowNum - BookingNo: $bookingNo, ContainerNo: $containerNo, Cargo status: Gate in (arrival at first POL)")
        } else if ((rowNum == 2) || (rowNum == 4)) {
            WebUI.verifyMatch(cargoStatusText, 'Gate in (Gate In to Outbound Terminal)', false, FailureHandling.STOP_ON_FAILURE)

            KeywordUtil.logInfo("Row $rowNum - BookingNo: $bookingNo, ContainerNo: $containerNo, Cargo status: Gate in (Gate In to Outbound Terminal)")
        }
    }
}
...
for (def rowNum2 = 1; rowNum2 <= 4; rowNum2++) {
    ...
    if (((rowNum2 == 1) || (rowNum2 == 2)) || (rowNum2 == 3)) {
        def bookingNo1text = testData.getValue('BookingNo', 1)

        def bookingNo2text = testData.getValue('BookingNo', 2)

        WebUI.setText(findTestObject('Scenario 1/S1_TC090/Page_Shipping Detail List - Brivge/input_Shipping Detail List'), 
            (bookingNo1text + ' ') + bookingNo2text)
