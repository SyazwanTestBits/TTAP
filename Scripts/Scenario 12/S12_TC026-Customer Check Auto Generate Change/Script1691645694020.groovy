...
int numberrowtd = findTestData('Scenario 12/SC12_TC026-Customer Check Auto Generate Change').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String col : columnname) {
        String valuecol = findTestData('Scenario 12/SC12_TC026-Customer Check Auto Generate Change').getValue(col, rowl)

        def actualValue = WebUI.getText(findTestObject('Scenario 12/SC12_TC026/p_partsDetail', [('lrow') : rowl, ('lcol') : coll]))

        KeywordUtil.logInfo("Expected value: $valuecol ; Actual value: $actualValue")

        WebUI.verifyElementText(findTestObject('Scenario 12/SC12_TC026/p_partsDetail', [('lrow') : rowl, ('lcol') : coll]), 
            valuecol)
