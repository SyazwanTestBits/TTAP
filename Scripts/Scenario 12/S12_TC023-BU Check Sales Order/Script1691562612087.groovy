...
WebUI.click(findTestObject('Scenario 12/SC12_TC023/p_SO Monitoring List'))
...
WebUI.scrollToElement(findTestObject('Scenario 12/SC12_TC023/div_2'), 0)

WebUI.click(findTestObject('Scenario 12/SC12_TC023/p_Parts No Sorting'))

int numberrowtd = findTestData('Scenario 12/SC12_TC023-BU Check Forecast SO').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String colname : columnname) {
        String valuecol = findTestData('Scenario 12/SC12_TC023-BU Check Forecast SO').getValue(colname, rowl)

        def actualValue = WebUI.getText(findTestObject('Scenario 12/SC12_TC023/p_partForecast', [('lrow') : rowl, ('lcol') : coll]))

        KeywordUtil.logInfo("Expected value: $valuecol ; Actual value: $actualValue")

        WebUI.verifyElementText(findTestObject('Scenario 12/SC12_TC023/p_partForecast', [('lrow') : rowl, ('lcol') : coll]), 
            valuecol)
