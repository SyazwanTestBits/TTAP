...
WebUI.callTestCase(findTestCase('Scenario 10/S10_TC166.1-Check CO_1'), [('CO_numberList') : [findTestData('Scenario 10/S10_TC160').getValue(
                2, 1), findTestData('Scenario 10/S10_TC160').getValue(2, 2)], ('contractNoL2') : findTestData(
            'Scenario 10/S10_TC021-Setup Data').getValue('ContractNo', 1), ('columnname') : ['PartsNo', 'CustomerPartsNo'
            , 'BackNo', 'SOnumber', 'SupplierCode', 'SPQ', 'OrderLot', 'OrderQty', 'ForceCompleteQty', 'UnitPrice', 'Currency'
            , 'Status', 'InboundedQty', 'InboundedPlanQty', 'InboundedPlanStatus']], FailureHandling.STOP_ON_FAILURE)

CustomKeywords.'util.ScrollToElement.clickUsingJS'(findTestObject('Scenario 10/S10_TC177/p_CO Monitoring List'), 0)

WebUI.callTestCase(findTestCase('Scenario 10/S10_TC166.2-Check CO_2'), [('CO_numberList') : [findTestData('Scenario 10/S10_TC160').getValue(
                2, 1), findTestData('Scenario 10/S10_TC160').getValue(2, 2)], ('contractNoL2') : findTestData(
            'Scenario 10/S10_TC021-Setup Data').getValue('ContractNo', 1), ('columnname') : ['PartsNo', 'CustomerPartsNo'
            , 'BackNo', 'SOnumber', 'SupplierCode', 'SPQ', 'OrderLot', 'OrderQty', 'ForceCompleteQty', 'UnitPrice', 'Currency'
            , 'Status', 'InboundedQty', 'InboundedPlanQty', 'InboundedPlanStatus']], FailureHandling.STOP_ON_FAILURE)
