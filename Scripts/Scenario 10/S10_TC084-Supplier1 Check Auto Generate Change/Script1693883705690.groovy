...
WebUI.setText(findTestObject('Page_ChangeCancelReqList/input_ChangeCancelReqList_Search'), contractRoute)

requestNo = WebUI.getText(findTestObject('Page_ChangeCancelReqList/div_DT_firstRequestNo'))

CustomKeywords.'copyToExcel.exel4'(requestNo, 1, 0, 'Excel Files/Scenario 10/S10_TestCases_Data.xlsx', 'TC084-Change Request No')
...
int numberrowtd = findTestData('Scenario 10/S10_TC084-Supplier1 Auto Generate Change').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String col : columnname) {
        String valuecol = findTestData('Scenario 10/S10_TC084-Supplier1 Auto Generate Change').getValue(col, rowl)
