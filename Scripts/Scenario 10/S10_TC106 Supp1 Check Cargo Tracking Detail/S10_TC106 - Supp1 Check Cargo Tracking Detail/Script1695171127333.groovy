...
for (int row2 = 1; row2 <= testData.getRowNumbers(); row2++) {
    ...
    for (String milestonecol : milestone) {
		...
		KeywordUtil.logInfo(mainforpath2 + ' Milestone: ' + milestonecol + ' Actual colour: ' + actualColour + ' expectation colour: ' + sytle2)
...
WebUI.callTestCase(findTestCase('Scenario 10/S10_TC106 Supp1 Check Cargo Tracking Detail/S10_TC106.1 - Supp1 Check Cargo Tracking Detail SO'), [('containerNumber') : ''
        , ('milestone') : ['Booking Submitted', 'Booking Confirmed', 'Empty Container Release to Shipper', 'Cargo Outbound'
            , 'Gate in (arrival at first POL)', 'Container loaded at first POL', 'Vessel departure from first POL', 'Vessel arrival at T/S port'
            , 'Container discharge at T/S port', 'Container loaded at T/S port', 'Vessel departure from T/S', 'Vessel arrived at final POD (ATA)'
            , 'Container unload at final POD', 'Container Gate out from final POD', 'Vessel arrived at final POD (ATA)', 'Container unload at final POD'
            , 'Container Gate out from final POD'], ('milestoneREALTIME') : ['Empty container pick-up (Empty Container Release to Shipper)'
        ...
        , ('testDataForecast') : '', ('testData') : findTestData('Scenario 10/S10_TC106_SO')], FailureHandling.STOP_ON_FAILURE)
