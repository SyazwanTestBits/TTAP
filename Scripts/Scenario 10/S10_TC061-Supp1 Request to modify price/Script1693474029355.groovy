...
WebUI.waitForElementVisible(findTestObject('Page_RequestList/h3_Request List'), 0)

WebUI.click(findTestObject('Scenario 10/S10_TC061/button_add request'))

WebUI.click(findTestObject('Scenario 10/S10_TC061/li_MODIFY PRICE'))

WebUI.waitForElementVisible(findTestObject('Scenario 10/S10_TC061/h3_Request Modify Price'), 0)
...
partIndex = CustomKeywords.'mapRowDatAndRowIndices.extractPartsWithIndices'(latestPath, 1, 7)

//int startrow = 6

for (int index = 1; index <= testData.getRowNumbers(); index++) {
    
	part=testData.getValue('Part No', index)
	
	rowindex=partIndex[part]
	...
	for (def pair : columnname) {
        ...
        CustomKeywords.'copyToExcel.exel3'(dataString, rowindex, columnIndex, latestPath, 'Contract Price')
    }
}
...
CustomKeywords.'util.ScrollToElement.clickUsingJS'(findTestObject('Scenario 10/S10_TC061/button_Submit'), 0)
...
CustomKeywords.'copyToExcel.exel4'(requestList, 1, 4, 'Excel Files/Scenario 10/S10_TestCases_Data.xlsx', 'TC061')
