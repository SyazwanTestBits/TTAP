...
WebUI.scrollToElement(findTestObject('Scenario 12/SC12_TC034/p_Parts No sorting'), 0, FailureHandling.STOP_ON_FAILURE)

WebUI.click(findTestObject('Scenario 12/SC12_TC034/p_Parts No sorting'))

int numberrowtd = findTestData('Scenario 10/S10_TC085-BU Auto Generate Change').getRowNumbers()

for (int rowl = 1; rowl <= numberrowtd; rowl++) {
    int coll = 1

    for (String col : columnname) {
        String valuecol = findTestData('Scenario 10/S10_TC085-BU Auto Generate Change').getValue(col, rowl)
