...
not_run: WebUI.verifyElementAttributeValue(findTestObject('Scenario 10/S10_TC008/View Contract/input__exportCParty'), 'value', 
    exportCParty, 0)

not_run: WebUI.verifyElementAttributeValue(findTestObject('Scenario 10/S10_TC008/View Contract/input__importCParty'), 'value', 
    importCParty, 0)

not_run: WebUI.verifyElementAttributeValue(findTestObject('Scenario 10/S10_TC008/View Contract/input__printHscodeFlag'), 
    'value', hsCode, 0)

not_run: WebUI.verifyElementAttributeValue(findTestObject('Scenario 10/S10_TC008/View Contract/input__incotermsCode'), 'value', 
    incotermsCode, 0)

not_run: WebUI.verifyElementAttributeValue(findTestObject('Scenario 10/S10_TC008/View Contract/input__incotermsPlace'), 
...
for (int row = 1; row <= testDataDT.getRowNumbers(); row++) {
    ...
    for (String colname : headerlist) {
		...
		if (colname == 'CurrentApplyDate') {
				expectdata = CustomKeywords.'DateConversionLocal.changeDateFormat_Chinese_Into_Chinese'(testDataDT.getValue(colname, row), 'MMM d, yyyy', 'yyyy年MM月dd日')
		}
