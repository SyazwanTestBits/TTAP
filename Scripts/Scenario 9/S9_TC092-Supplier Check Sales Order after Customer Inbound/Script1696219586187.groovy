...
for (def index : (1..datafile_outbound.getRowNumbers())) {
    def deliveredQty = datafile_outbound.getValue('DeliveredQty', index)

    def inTransitQty = datafile_outbound.getValue('InTransitQty', index)

    def formattedDeliveredQty = deliveredQty.replaceAll('(\\d)(?=(\\d{3})+(?!\\d))', '$1,')

    def formattedInTransitQty = inTransitQty.replaceAll('(\\d)(?=(\\d{3})+(?!\\d))', '$1,')

    WebUI.verifyElementText(findTestObject('Scenario 13/S13_TC050 TC053/partsDetail_Status', [('row') : index]), mapverify_partsdetail[
        index])

    WebUI.verifyElementText(findTestObject('Scenario 13/S13_TC050 TC053/partsDetail_DeliveredQty', [('row') : index]), formattedDeliveredQty)

    WebUI.verifyElementText(findTestObject('Scenario 13/S13_TC050 TC053/partsDetail_InTransitQty', [('row') : index]), formattedInTransitQty)
}
