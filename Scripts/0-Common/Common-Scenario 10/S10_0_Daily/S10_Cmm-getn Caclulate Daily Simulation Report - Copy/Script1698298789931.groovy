...
for (int index = 1; index <= testDataL2.getRowNumbers(); index++) {
	partNo = testDataL2.getValue('Part No', index)

	rowIndex = (partIndex[partNo])
	
	rowIndexL3 = (partIndex[testData.getValue('PartNo', 1)])
	
	int DateColumn = 34
	
	String dateUsage = CustomKeywords.'util.compareTestData.getCellValue2'(actualPath, rowIndexL3 - 3, DateColumn, 0)
	
	def dateMapIn=[:]
	
	while (dateUsage != 'No Usage') {
		
		stockQty=CustomKeywords.'util.compareTestData.getCellValue2'(actualPath, rowIndex - 1, DateColumn, 0)
