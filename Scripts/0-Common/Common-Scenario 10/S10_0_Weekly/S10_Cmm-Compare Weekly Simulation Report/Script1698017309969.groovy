...
for (int row = 1; row <= expectationPath.getRowNumbers(); row++) {
    partNo = expectationPath.getValue('Part No', row)
    ...
    String firstWeekBalance = CustomKeywords.'util.compareTestData.getCellValue2'(actualPath, rowIndex, 31, 0)

    if (firstWeekBalance == '0.0') {
        columnNameWeek = columnName2.clone()
    } else {
        columnNameWeek = columnName1.clone()
    }
    
    for (def pair : columnNameWeek) {
        def columnName = pair.key
