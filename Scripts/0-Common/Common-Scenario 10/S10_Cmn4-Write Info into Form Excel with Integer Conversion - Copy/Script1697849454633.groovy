...
for (def index : (1..datafile.getRowNumbers())) {
	...
	String partNo= datafile.getValue('Parts No.', index)
	
	def dataIndex= partIndexMap[partNo]
    ...
    for (def pair : fileColumns) {
        ...
        CustomKeywords.'copyToExcel.exel4'(dataValue, dataIndex, columnIndex, downloadedFormPath, downloadedFormSheetname)
    }
}
