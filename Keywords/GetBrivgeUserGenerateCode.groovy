...
public class GetBrivgeUserGenerateCode {

	// Stores the extracted verification codes
	private static String userGenCodes;
	...
	@Keyword
	static String extractUserGenCode() {
		...
		Message[] messages = inbox.search(new SubjectTerm("BriVge User Generate"))
		...
		if (messages.length > 0) {
			...
			try {
				...
				// Define the username (replace 'tb-qa' with the actual username)
				def username_temp = GlobalVariable.TEMP_USERNAME

				// Define the regular expression pattern with the username placeholder
				def pattern = ~/(?<=Your password for account, $username_temp )(.*?)(?= This)/

				// Use find to match the pattern in the text
				def matcher = (extractedText =~ pattern)

				// Extract the desired portion if a match is found
				if (matcher.find()) {
					def text = matcher.group(1)
					KeywordUtil.logInfo("Pattern found: $text")
					userGenCodes = text
				} else {
					KeywordUtil.logInfo("Pattern not found in the text.")
				}
